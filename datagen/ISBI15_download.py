...
file1 = wget.download('http://cs.adelaide.edu.au/~zhi/isbi15_challenge/Training_R1_01Dec2014.zip')
file2 = wget.download('http://cs.adelaide.edu.au/~zhi/isbi15_challenge/Testing_R1_01Dec2014.zip')
file3 = wget.download('http://cs.adelaide.edu.au/~zhi/isbi15_challenge/Training_R2_Jan2015.zip')
file4 = wget.download('http://cs.adelaide.edu.au/~zhi/isbi15_challenge/Testing_R2_Jan2015.zip')
file5 = wget.download('http://cs.adelaide.edu.au/~zhi/isbi15_challenge/AnnotationTest.mat')

with zipfile.ZipFile(file1, 'r') as zip:
    for x in ['frame004.png', 'frame011.png', 'frame013.png', 'frame014.png']:
        zip.extract('Training/EDF/%s' %x, 'data/ISBI15/')

with zipfile.ZipFile(file3, 'r') as zip:
    for x in ['frame007.png', 'frame010.png', 'frame016.png', 'frame017.png']:
        zip.extract('EDF/%s' %x, 'data/ISBI15/Training')

with zipfile.ZipFile(file2, 'r') as zip:
    for x in ['frame000.png', 'frame001.png', 'frame002.png', 'frame015.png']:
        zip.extract('Testing/EDF/%s' %x, 'data/ISBI15/')

with zipfile.ZipFile(file4, 'r') as zip:
    for x in ['frame018.png', 'frame019.png', 'frame020.png', 'frame021.png', 'frame022.png']:
        zip.extract('EDF/%s' %x, 'data/ISBI15/Testing')

with zipfile.ZipFile(file1, 'r') as zip:
    for y in zip.namelist():
        if y.startswith("Training/seg_frame"):
            zip.extract(y, "data/ISBI15/Training_annotations/")

with zipfile.ZipFile(file3, 'r') as zip:
    for y in zip.namelist():
        if y.startswith("seg_frame"):
            zip.extract(y, "data/ISBI15/Training_annotations/Training")

os.rename('AnnotationTest.mat', 'data/ISBI15/AnnotationTest.mat')

# remove zip files after extraction
os.remove("Testing_R1_01Dec2014.zip")
os.remove("Testing_R2_Jan2015.zip")
os.remove("Training_R1_01Dec2014.zip")
os.remove("Training_R2_Jan2015.zip")

# locations of images and masks
trainset_filepath = 'data/ISBI15/Training/EDF'
testset_filepath = 'data/ISBI15/Testing/EDF'
trainset_GT_filepath = 'data/ISBI15/Training_annotations/Training'
testset_GT_filepath = 'data/ISBI15/AnnotationTest.mat'
